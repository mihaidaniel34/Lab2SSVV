...
public class Service {
    ...
    public Student updateStudent(Student student){
        studentValidator.validate(student);
        if (studentFileRepository.update(student) == null) {
            return student;
        }
        return null;
    }
    ...
    public Tema updateTema(Tema tema){
        temaValidator.validate(tema);
        if (temaFileRepository.update(tema) == null) {
            return tema;
        }
        return null;
    }
    ...
    public Double addNota(Nota nota, String feedback){
        ...
        if(predare > tema.getDeadline()){
            if (predare - tema.getDeadline() <= 2){
                nota.setNota(nota.getNota()- (2.5 * (predare - tema.getDeadline())));
            }
            else{
                throw new ValidationException("Studentul nu mai poate preda aceasta tema!");
            }
        }
        Nota notaSaved = notaFileRepository.save(nota);
        if (notaSaved != null) {
            return nota.getNota();
        }
